...
http_nodogsplash_make_authtarget(char* token, char* redir)
{
	...
	else
	{
		...
		if(strlen(config->UID)  && strlen(public_ip) )
			...
			safe_asprintf(&(authtarget->authtarget), "http://%s/getservice.asp?DeviceID=%s&UserIdentification=%s&ReturnUrl=http://%s:8888", config->FBServerURL,config->UID,encodedtok,public_ip);
